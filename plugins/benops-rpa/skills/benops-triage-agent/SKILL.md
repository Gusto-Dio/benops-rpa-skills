...
**That companion skill is installed separately and does not ship in this plugin.** Check it is present before you start. If it is missing, say so plainly and stop — improvising `uip` command shapes against the Production tenant is exactly the risk this skill exists to remove.
