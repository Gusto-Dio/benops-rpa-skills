...
### Changed
- `ticket-fields`: writes **one field per call in the team's stated order** instead of one combined `editJiraIssue`, which gives Jira no ordering at all and is what leaves a ticket fully filled with no dates. Order is Priority → Process → Complexity → Story Points → **Status (`In Progress`, transition 41)** → Sprint, Ticket Type? → Workaround Solutions, Category of Break
- `ticket-fields`: added a **Due date repair pass**. The Due date rule triggers on a Story Points change and writes `duedate = Start date + Story Points days`; Start date only exists after the transition, so in this order the first Story Points write is skipped and does not retry. The skill now re-writes Story Points once after the transition and reports honestly if the Due date still does not appear — it never hand-writes `duedate`, which would be indistinguishable from the automation's in reports
- `ticket-fields`: the verify read covers `customfield_10015` (Start date) and `duedate` as well as the eight, allowing for the ~2.2–2.7 s automation latency
